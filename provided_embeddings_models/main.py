...
def get_age_group_split(data: pd.DataFrame) -> Tuple[
    ...
    return split_embeddings(age_group_cleaned, "age_group", val_size=0.12)
...
def get_age_split(data: pd.DataFrame) -> Tuple[
    ...
    return split_embeddings(age_cleaned, "age", val_size=0.12)
...
def get_gender_split(data: pd.DataFrame) -> Tuple[
    ...
    return split_embeddings(gender_cleaned, "gender", val_size=0.12)
...
def train_model_main(task: TaskType, cfg: TaskConfig, data: pd.DataFrame) -> None:
    # Split and scale embeddings ##################################################
    if task == "age_group":
        raw_train, raw_val, raw_test, train_labels, val_labels, test_labels = get_age_group_split(data)
    elif task == "gender":
        raw_train, raw_val, raw_test, train_labels, val_labels, test_labels = get_gender_split(data)
    else:
        raw_train, raw_val, raw_test, train_labels, val_labels, test_labels = get_age_split(data)

    scaled_train, scaled_val, scaled_test = standardize_features(raw_train, raw_val, raw_test)
...
if __name__ == "__main__":
    # Task selection ##############################################################
    TASK: TaskType = "age"  # MUST BE "age_group" | "gender" | "age"
    ...
    all_data = load_embeddings_data(YAMNET_FILENAME)

    train_model_main(TASK, task_config, all_data)
